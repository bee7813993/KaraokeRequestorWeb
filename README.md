...
カラオケのみに限らず、参加者がBGMのような音楽や動画を選択するようなイベントでも使用できる
...
## 画面・操作性
- スマホで使いやすいモダンなUI(Bootstrap 5)。従来デザイン(Bootstrap 3)も引き続き使用可能で、設定で切り替えられます
...
- **WiFiアクセスポイント** — ローカル運用時のみ必要。少人数の場合スマホテザリングでも可(オンライン接続機能利用時は不要)
