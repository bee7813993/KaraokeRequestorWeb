...
- 環境設定の「クール一覧データ同期」欄に検索先の保存フォームを追加し、クール一覧から開く検索先を `ゆかりすたー` / `Everything` で切り替え可能にした。
...
- 検索先切替は `setlist_search_backend` の設定値とJSONレスポンスで制御し、フロント側は値に応じてURL生成のみを切り替える。設定UIは同期ボタンと同じ欄に置き、保存場所が分散しないようにしている。
...
環境設定のクール一覧データ同期欄から最新データ同期を実行できるほか、同じ欄でクール一覧から曲を開く際の検索先を `ゆかりすたー` / `Everything` で切り替えられるようにしました。
